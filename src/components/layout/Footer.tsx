          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            ...
            <div className="col-span-2">
              ...
              <div className="flex gap-3 mt-5">
                {[
                  ...
                  { label: 'YouTube', svg: <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg> },
                ].map((social, i) => (
                  <a key={i} href="#" className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center hover:bg-primary transition-colors" title={social.label}>
                    {social.svg}
                  </a>
                ))}
              </div>
            ...
            {FOOTER_PRODUCTS.map((group) => (
              <div key={group.category}>
                <h3 className="font-semibold text-sm mb-3 text-white">{group.category}</h3>
            ...
            {/* Company */}
            <div>
              <h3 className="font-semibold text-sm mb-3 text-white">Company</h3>
              <ul className="space-y-2">
                {[
                  ...
                  <li key={link.href}>
                    <Link href={link.href} className="text-xs text-white/40 hover:text-white transition-colors flex items-center gap-1 group">
                      {link.label}
                    </Link>
                  </li>
                ))}
            ...
            {/* Support */}
            <div>
              <h3 className="font-semibold text-sm mb-3 text-white">Support</h3>
              <ul className="space-y-2">
                {[
        ...
        <div className="border-t border-white/10 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
              ...
              <p className="text-xs text-white/40">Get exclusive offers, design tips, and printing guides delivered to your inbox.</p>
            </div>
            <div className="flex gap-2 w-full md:w-auto">
              <input
                type="email"
                placeholder="Enter your email"
                className="flex-1 md:w-72 px-4 py-2.5 bg-white/10 rounded-lg border border-white/10 text-sm text-white placeholder:text-white/30 outline-none focus:border-primary"
              />
        ...
        <Container>
          <div className="py-5 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <span className="text-[10px] text-white/30 uppercase">We Accept</span>
              <div className="flex gap-1.5">
                {['Visa', 'MC', 'UPI', 'PayTM', 'GPay'].map((p) => (
                  <span key={p} className="px-2 py-1 bg-white/10 text-[9px] text-white/50 rounded font-medium">{p}</span>
                ))}
              </div>
            </div>
          </div>
